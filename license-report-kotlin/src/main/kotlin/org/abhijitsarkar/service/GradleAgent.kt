...
internal class GradleAgentImpl(vararg val tasks: String = arrayOf("clean", "downloadLicenses")) : GradleAgent {
    ...
    override fun generateLicense(dir: File, options: String): Mono<Pair<String, String>> {
        return gradleDistroUrl(dir)
                .flatMapMany { connection(dir, it) }
                .map { connection ->
    ...
    private fun connection(dir: File, url: String): Mono<ProjectConnection> {
        return Mono.just(GradleConnector.newConnector()
    ...
    internal fun gradleDistroUrl(dir: File): Mono<String> {
