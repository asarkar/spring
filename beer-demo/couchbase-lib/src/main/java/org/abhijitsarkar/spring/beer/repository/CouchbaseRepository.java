 ...
 * Better than CouchbaseAsyncBucket, able to delay resolving to bucket.
 */
public interface CouchbaseRepository<T> {
    ...
    default Observable<T> findByIds(final List<String> ids) {
        if (CollectionUtils.isEmpty(ids)) {
            return Observable.empty();
        }
        return asyncBucket()
                .flatMapObservable(bucket -> {
                    SimpleN1qlQuery query = N1qlQuery.simple(
                            select(path(i(bucket.name()), "*"))
                                    .from(i(bucket.name()))
                                    .useKeys(JsonArray.from(ids))
                    );
                    return queryExecutor().apply(bucket, query);
                })
                .doOnError(t -> LOGGER.error("Failed to query docs with ids: {}.", ids, t))
                .flatMap(row -> convert(row.value().toString()).toObservable());
    }

    default Observable<T> findByField(String fieldName, String fieldValue) {
        return findByFieldInIds(emptyList(), fieldName, fieldValue);
    }

    default Observable<T> findByFieldInIds(final List<String> ids, String fieldName, String fieldValue) {
        return asyncBucket()
                .flatMapObservable(bucket -> {
